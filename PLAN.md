...
| 模型切换 GUI + pi 插件 + 技能展示（M4 后新增） | 不可在 GUI 改模型；插件/技能无入口 | ①标题栏模型 chip + 弹窗选择器（`models_list/model_set/thinking_set`，模型清单按 provider 鉴权过滤，`setModel` 自带 settings 持久化，`model_changed` 事件推送）；②侧边栏改「会话/设置」双标签，设置页=技能只读展示（`skills_list`）+ pi 插件安装/移除（`PackageManager.installAndPersist/removeAndPersist` + `resourceLoader.reload()`）；③**资源隔离修复**：skills/prompts/themes/extensions 四类 override 过滤至 agentDir 内，杜绝 `~/.agents/skills` 宿主技能泄漏（实测修复后仅剩 kairo-skills）；④内置 `kairo-skills` 技能（repo `skills/`，install.sh 同步 + daemon 启动兜底）教模型自行安装技能 | 用户需求：GUI 改模型、pi 插件功能、skills 只读展示+文档（docs/skills.md） |
| 提供商 GUI（M4 后新增） | 手改 auth/models-store/settings 三文件 | **侧边栏「设置」页新增提供商区**：列表（id/鉴权/模型数/移除，当前在用不可删）+ 添加表单（id + api key + 可选 baseUrl）。自定义 OpenAI 兼容 provider 自动探测 `/models` 生成目录；**写入 `models.json`（权威定义，SDK 启动时组合；models-store.json 只是原生 provider 目录缓存，写它无效——探针验证）** + auth.json（内联 apiKey 双保险），随后延迟 600ms 自重启 systemctl（refresh()/setRuntimeApiKey 均无法让新 provider 即时可见） | 用户需求：GUI 添加提供商 |
| 自动命名清屏修复（M4 后） | 命名广播 session_active | **根因**：`maybeAutoName` 命名成功后广播 `session_active`，UI 收到即 `resetMessages()`——第一轮对话刚结束就被清空，看起来像全新对话（切走再切回才恢复）。修复：①命名后改广播 `status`（刷新标题栏名字，不清视图）；②ChatPanel 的 `session_active` 仅在会话 id 变化时才清屏（`_shownSessionId` 防御同会话重复事件） | 用户报告：新对话聊完结果突然消失，切走切回才回来 |
| 命名后流式状态卡死修复（M4 后） | 命名广播 status 带旧 streaming | **根因**：`maybeAutoName` 在 `agent_end` 回调内广播 `status()`，而 SDK 的 `isStreaming` 在事件派发后才置 false（`_emitAgentSettled`）——广播把 streaming=true 带回 UI，状态点永远黄色、发送按钮卡在「中止」。修复：命名后先 `waitUntilIdle`（轮询 isStreaming，3s 超时），空闲才广播 status；一直忙则跳过，名字由 session_list/下次 status 承担 | 用户报告：输出完后 UI 一直显示黄色+中止 |
**跟进修复**：①✕ 仅显示在可移除（models.json/auth/models-store 有记录）且非当前的提供商上，SDK 内置提供商不再出现无效 ✕；②添加表单改为带标签行（id / api key / baseUrl），key 密码态输入；③插件安装/提供商添加加忙碌态反馈（按钮变「安装中…/添加中…」并禁用，20s 看门狗兜底） |
