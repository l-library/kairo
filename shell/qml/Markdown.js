...
function inline(md) {
  ...
  t = t.replace(/`([^`]+)`/g, function (_, c) {
    codes["@@C" + ci + "@@"] = '<code style="font-family:monospace;background:#313244;padding:1px 4px;border-radius:3px;">' + escapeHtml(c) + "</code>"
    return "@@C" + ci++ + "@@"
  })
  // 粗体
  t = t.replace(/\*\*([^*]+)\*\*/g, "<b>$1</b>")
  ...
  // 链接 [text](url)
  t = t.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" style="color:#89b4fa;text-decoration:none;">$1</a>')
  // 恢复行内代码
  for (var k in codes) {
 ...
 * palette: { codeBg: "#17171e", accent: "#89b4fa", muted: "#a6adc8" }
 */
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (/^```/.test(line)) {
      if (inCode) {
        ...
        html += '<pre style="font-family:monospace;background:' + codeBg + ';padding:8px 10px;border-radius:6px;color:#cdd6f4;white-space:pre-wrap;"><code>'
        inCode = true
      }
      continue
    }
    ...
    if (/^\|.*\|$/.test(line)) {
      ...
      if (tableHead === null && !/^[\s:|-]+$/.test(line)) {
        ...
        for (var c1 = 0; c1 < cells.length; c1++) html += '<th style="border-bottom:1px solid #45475a;text-align:left;">' + inline(cells[c1]) + "</th>"
        html += "</tr>"
        continue
      }
      if (/^[\s:|-]+$/.test(line)) continue // 分隔行
      html += "<tr>"
      for (var c2 = 0; c2 < cells.length; c2++) html += '<td style="border-bottom:1px solid #45475a;">' + inline(cells[c2]) + "</td>"
      html += "</tr>"
      continue
    }
    ...
    if (h) {
      ...
      html += '<div style="font-size:' + size + ';font-weight:bold;margin:6px 0 2px 0;">' + inline(h[2]) + "</div>"
      continue
    }
    ...
    if (/^>\s?/.test(line)) {
      ...
      html += "<div>" + inline(line.replace(/^>\s?/, "")) + "</div>"
      continue
    } else { closeQuote() }
    ...
    if (oli) {
      ...
      html += '<li style="margin:2px 0;">' + inline(oli[1]) + "</li>"
      continue
    }

    // 分隔线
    if (/^(-{3,}|\*{3,})$/.test(line.trim())) {
      html += '<hr style="border:none;border-top:1px solid #45475a;margin:6px 0;"/>'
      continue
    }
